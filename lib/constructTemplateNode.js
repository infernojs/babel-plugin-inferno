...
function hasHyphonOrColon(attr) {
			...
			if (attrs.index !== undefined) {
				...
				if (attrKeys.length > 0) {
					template.push(t.ObjectProperty(t.identifier('attrs'), t.ObjectExpression(attrKeys.map(function (attrKey) {
						return t.ObjectProperty(hasHyphonOrColon(attrKey) ? t.StringLiteral(attrKey) : t.identifier(attrKey), getObjectValue(t, attrs[attrKey]));
					}))));
				}
