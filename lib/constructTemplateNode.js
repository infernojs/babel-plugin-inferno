...
function constructTemplateNode(t, node) {
	...
	if (tag) {
		...
		if (!isNullOrUndefined(attrs)) {
			if (typeof attrs.index === 'number') {
				...
				if (tag.value !== 'svg' && (attrs['class'] !== undefined && tag.type === 'StringLiteral' || attrs.className !== undefined && tag.type === 'StringLiteral')) {
					template.push(t.ObjectProperty(t.identifier('className'), getObjectValue(t, attrs['class'] || attrs.className)));
					delete attrs['class'];
				...
				if (attrKeys.length > 0 || (spreadKeys && spreadKeys.length > 0)) {
					const attrsObject = t.ObjectExpression(attrKeys.map(function (attrKey) {
						...
						if (instance !== null && attrsSVG[proposedAttr] === false) {
