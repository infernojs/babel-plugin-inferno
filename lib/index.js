...
function createTemplateProps(t, attributes, templateState, component, hash) {
			...
			case 'JSXAttribute':
				...
				if (value) {
					if (value.type === 'JSXExpressionContainer') {
						...
						hash += `|prop:${ name }=${ expressionToString(expression) }`;
	...
	return { staticProps, dynamicProps, hooks, key, ref, childrenType, newHash: hash };
}
...
function createNode(t, node, bps, templateState, hash) {
	switch (node.type) {
		case 'JSXElement':
			...
			const { staticProps, dynamicProps, key, ref, hooks, childrenType, newHash } = createTemplateProps(t, attributes, templateState, component, hash);

			if (component) {
				const { dynamicChildren, staticChildren } = createChildren(t, node.children, bps, templateState, newHash);
				...
				return { isStatic: false, node: createVElement(t, bps, name, key, ref, staticProps, dynamicProps, dynamicChildren, staticChildren, childrenType, templateState, newHash) };
			}
		case 'JSXText':
