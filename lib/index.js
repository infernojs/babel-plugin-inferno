...
function createChildren(t, nodeChildren, args, params, templateState) {
	const children = [];

	for (let i = 0; i < nodeChildren.length; i++) {
		const { node } = createNode(t, nodeChildren[i], args, params, templateState, false);
...
function createNode(t, node, args, params, templateState, isRoot) {
	switch (node.type) {
		case 'JSXElement':
			...
			if (component) {
				const children = createChildren(t, node.children, args, params, templateState);
		...
		visitor: {
			JSXElement: {
				enter(path, scope) {
					...
					const { skipTemplate, node } = createNode(t, path.node, args, params, templateState, true);
