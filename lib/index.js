		...
		if (Array.isArray(hooks)) {
			if (hooks.length > 0) {
				return t.ObjectExpression(hooks.map(
					(hook) => hook.spread
						? t.SpreadProperty(hook.value)
						: t.ObjectProperty(
							typeof hook.name === 'string' ? t.identifier(hook.name) : hook.name,
							hook.value
						)
				));
			}
...
function createVElement(
	...
	if (useInline) {
		return t.ObjectExpression([
			...
			t.ObjectProperty(t.identifier('nodeType'), t.memberExpression(t.Identifier(shortnames.NodeTypes), t.identifier('OPT_ELEMENT')))
		].concat(bpValues));
	} else {
...
function createVComponent(t, component, key, ref, props, hooks, templateState) {
	if (templateState.useInline) {
		...
		return t.ObjectExpression([
			...
			t.ObjectProperty(t.identifier('key'), key || t.identifier('null')),
			t.ObjectProperty(t.identifier('nodeType'), t.memberExpression(t.Identifier(shortnames.NodeTypes), t.identifier('COMPONENT'))),
			t.ObjectProperty(t.identifier('props'), propsObject),
			t.ObjectProperty(t.identifier('ref'), ref || t.identifier('null')),
			t.ObjectProperty(t.identifier('type'), component)
		]);
	} else {
		const args = [
			component
...
function createChildren(t, nodeChildren, bps, templateState, hashBuiler, forceDynamicChildren) {
	...
	return { allChildrenAreVNodes, staticChildren, dynamicChildren, childrenHash: isDynamic ? dynamicChildrenHashes.join('') : staticChildrenHashes.join('') };
}
		...
		visitor: {
			JSXElement: {
				enter(path, state) {
					...
					addTemplateToModule(t, getHoistedNode(path.node, path.parentPath), bps, opts.addedShorthands, libName, opts.pragmaMethod);
					if (bps.length > 0) {
