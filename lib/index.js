...
function addCreateVNodeImportStatement(t, toInsert, opts) {
	...
	if (opts.imports !== false) {
		node.body.splice(index, 0, t.importDeclaration([
			t.ImportSpecifier(t.identifier('normalizeProps'), t.identifier(opts.pragma || 'normalizeProps')),
			t.ImportSpecifier(t.identifier('createTextVNode'), t.identifier(opts.pragmaTextVNode || 'createTextVNode')),
			t.ImportSpecifier(t.identifier('normalize'), t.identifier(opts.pragmaNormalize || 'normalize')),
		...
		node.body.splice(index, 0, t.VariableDeclaration('var', [
			...
			t.VariableDeclarator(
				t.Identifier('normalize'),
				t.memberExpression(t.identifier('Inferno'), t.identifier('normalize'))
			),
			t.VariableDeclarator(
...
function getVNodeChildren(t, astChildren, opts, forceNoNormalize) {
	var children = [];
	...
	return {
		...
		doNormalize: requiresNormalization && !forceNoNormalize,
		foundTextInArray: !hasSingleChild && foundText
	};
}
...
function addNormalizeCall(vChildren, t, opts) {
...
function createVNode(t, astNode, opts) {
	...
	case 'JSXElement':
		...
		var propInfo = getVNodeProps(t, openingElement.attributes, vType.isComponent);
		var vProps = propInfo.value;
		var childrenResults = getVNodeChildren(t, astNode.children, opts, vProps.noNormalize);
		...
		if (vType.isComponent && vChildren) {
			...
			if (addChildrenToProps) {
				if (childrenResults.doNormalize) {
					vChildren = t.callExpression(
						t.identifier(opts.pragmaNormalize || 'normalize'),
						[vChildren]
					);
				} else if (childrenResults.foundTextInArray && vChildren.elements) {
					addNormalizeCall(vChildren, t, opts);
				}
				if (props.properties) {
		...
		if (propInfo.needsNormalization) {
			return t.callExpression(
				t.identifier(opts.pragma || 'normalizeProps'),
				[createVNodeCall]
			);
		}
