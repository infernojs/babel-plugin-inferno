...
function createVElement(
	...
	let { hashMap, bpCounter, useInline } = templateState;
	const bpValues = [];
	const bpDescriptors = [];
	let children;
	...
	if (useInline) {
		...
		].concat(bpValues.map(({ value }) => value)));
	}
}

function createVComponent(t, component, key, ref, props, hooks, { useInline }) {
	if (useInline) {
		let propsObject;
...
function createChildren(t, nodeChildren, bps, templateState, hashBuiler, forceDynamicChildren) {
	...
	for (let i = 0; i < nodeChildren.length; i++) {
		...
		const { isVNode, node, isStatic, empty } = createNode(t, nodeChildren[i], bps, templateState, childHashBuiler, false);
...
function createNode(t, node, bps, templateState, hashBuiler, isRoot) {
	switch (node.type) {
		case 'JSXElement':
			...
			if (component) {
				const { dynamicChildren, staticChildren } = createChildren(t, node.children, bps, templateState, hashBuiler, true);
				...
				const { allChildrenAreVNodes, dynamicChildren, staticChildren, childrenHash } = createChildren(t, node.children, bps, templateState, hashBuiler, false);
		...
		visitor: {
			JSXElement: {
				enter(path, state) {
					...
					const { node } = createNode(t, path.node, bps, templateState, { hash: 'tpl' }, true);

					path.replaceWith(node);
					addTemplateToModule(t, getHoistedNode(path.node, path.parentPath), bps, libName, opts.pragmaMethod, opts.preclone);
					if (bps.length > 0) {
						opts.bps = [];
					}
				}
