...
function addCreateVNodeImportStatement(t, toInsert, opts) {
	...
	if (optionsImports !== false && optionsImports !== 'false') {
		node.body.splice(index, 0, t.importDeclaration([
			t.ImportSpecifier(t.identifier('normalizeProps'), t.identifier(opts.pragmaNormalizeProps || 'normalizeProps')),
			t.ImportSpecifier(t.identifier('createTextVNode'), t.identifier(opts.pragmaTextVNode || 'createTextVNode')),
			t.ImportSpecifier(t.identifier('normalizeChildren'), t.identifier(opts.pragmaNormalizeChildren || 'normalizeChildren')),
			t.ImportSpecifier(t.identifier('createComponentVNode'), t.identifier(opts.pragmaCreateComponentVNode || 'createComponentVNode')),
			t.ImportSpecifier(t.identifier('createVNode'), t.identifier(opts.pragma || 'createVNode')),
		], t.stringLiteral(typeof optionsImports === 'string' ? optionsImports : 'inferno')));
	} else if (!opts.pragma) {
...
function createVNode(t, astNode, opts) {
	...
	case 'JSXElement':
		...
		} else if (!childrenResults.requiresNormalization || vProps.noNormalize) {
