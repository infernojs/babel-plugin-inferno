...
function createVElement(
	...
	if (ref) {
		...
		hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_REF`;
	}
	for (let i = 0; i < dynamicProps.length; i++) {
		...
		if (spread) {
			...
			hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_SPREAD`;
		} else {
			if (name === 'className') {
				pushDescriptors(
					t,
				...
				hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_CLASS_NAME`;
			} else if (name === 'style') {
				pushDescriptors(
					t,
					bpDescriptors,
				...
				hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_STYLE`;
			} else if (name === 'value') {
				pushDescriptors(
					t,
					bpDescriptors,
					t.memberExpression(t.memberExpression(t.identifier('Inferno'), t.identifier('ValueTypes')), t.identifier('PROP_VALUE')),
					t.identifier('null')
				);
				hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_VALUE`;				
				...
				hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP_DATA`;
			} else {
				pushDescriptors(
					t,
					bpDescriptors,
				...
				hash += `|bp:v${ (bpValues.length - 1) }=Inferno.ValueTtpes.PROP:'${ name }'`;
			}
		}
	}
...
function createChildren(t, nodeChildren, bps, templateState) {
	...
	for (let i = 0; i < nodeChildren.length; i++) {
		...
		const { node, isStatic } = createNode(t, nodeChildren[i], bps, templateState, childHashBuiler);
...
function createNode(t, node, bps, templateState, hashBuiler) {
	switch (node.type) {
		case 'JSXElement':
			...
			if (component) {
				const { dynamicChildren, staticChildren } = createChildren(t, node.children, bps, templateState, hashBuiler);
				...
				const { dynamicChildren, staticChildren, childrenHash } = createChildren(t, node.children, bps, templateState, hashBuiler);

				return { isStatic: false, node: createVElement(t, bps, name, key, ref, staticProps, dynamicProps, dynamicChildren, staticChildren, childrenType, templateState, hashBuiler, childrenHash) };
			}
		case 'JSXText':
		...
		visitor: {
			JSXElement: {
				enter(path, scope) {
					...
					const { node } = createNode(t, path.node, bps, templateState, { hash: 'tpl' });
