...
function createChildren(t, nodeChildren, bps, templateState, libName) {
	...
	let dynamicChildren = [];
	let isDynamic = false;
	let allChildrenAreNodes = true;

	for (let i = 0; i < nodeChildren.length; i++) {
		const childHashBuiler = { hash: 'children' };
		const { node, isStatic, empty } = createNode(t, nodeChildren[i], bps, templateState, childHashBuiler, libName, false);
		...
		if (node) {
			...
			allChildrenAreNodes = false;
		}
	}
	return { allChildrenAreNodes, staticChildren, dynamicChildren, childrenHash: !isDynamic ? childrenHashes.join('') : '' };
}
	...
	switch (node.type) {
		case 'JSXElement':
			...
			if (component) {
				...
				const { allChildrenAreNodes, dynamicChildren, staticChildren, childrenHash } = createChildren(t, node.children, bps, templateState, libName);
				...
				if (isStatic && !isRoot) {
					return {
						isStatic: true,
						empty: false,
						node: createStaticVElement(t, name, staticProps, staticChildren, libName)
					};
				} else {
					return {
						isStatic: false,
		...
		case 'JSXText':
			...
			return { node: null, isStatic: true, empty: true };
		case 'JSXExpressionContainer':
			...
			return { node: expression, isStatic, empty: false };
		default:
			return { node: null, isStatic: true, empty: true };
	}
}
