...
function handleWhiteSpace(str) {
	const startingWhiteSpace = /^\s+/;
	const endingWhiteSpace = /\s+$/;
	const hasFinalSpaceChar = / $/.test(str);
	const allWhiteSpaceClusters = /\s+/g;

	return str
		.replace(startingWhiteSpace, '')
		.replace(endingWhiteSpace, hasFinalSpaceChar? ' ': '')
		.replace(allWhiteSpaceClusters, ' ');
}
