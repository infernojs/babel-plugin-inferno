...
function createTemplateProps(t, attributes, args, params, templateIndexes, component) {
	...
	return { props, key, ref, childrenType };
}
...
function createNode(t, node, args, params, templateIndexes, isRoot) {
	switch (node.type) {
		case 'JSXElement':
			...
			const { props, key, ref, childrenType } = createTemplateProps(t, attributes, args, params, templateIndexes, component);
			...
			if (component) {
				...
				if (params && !isRoot) {
					...
					return { skipTemplate: true, node: createVComponent(t, name, key, props) };
				}
			} else {
