  ...
  for (var i = 0; i < astProps.length; i++) {
    ...
    if (astProp.type === 'JSXSpreadAttribute') {
      ...
      if (!isComponent && (propName === 'className' || propName === 'class')) {
        ...
        case 'noNormalize':
        case '$NoNormalize':
          throw path.buildCodeFrameError('Inferno JSX plugin:\n' + propName + ' is deprecated use: $HasVNodeChildren, or if children shape is dynamic you can use: $ChildFlags={expression} see inferno package:inferno-vnode-flags (ChildFlags) for possible values');
...
function createVNode(t, astNode, opts, fileState, path) {
  ...
  case 'JSXElement':
    ...
    if (vProps.childFlags) {
      // If $ChildFlags is provided it is runtime dependant
