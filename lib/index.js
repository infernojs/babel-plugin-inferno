...
function getVNodeType(astNode, fileState) {
  ...
  if (astType === 'JSXIdentifier') {
    ...
    if (isFragment(astName)) {
      ...
      // TODO: Fix
      // flags = fileState.get(astName) || VNodeFlags.ComponentUnknown;
...
function getVNodeChildren(astChildren, opts, fileState) {
  var children = [];
  ...
  for (var i = 0; i < astChildren.length; i++) {
    ...
    var vNode = createVNode(child, opts, fileState);
...
function createVNodeArgs(flags, type, className, children, childFlags, props, key, ref) {
  var args = [];
...
function createVNode(astNode, opts, fileState) {
  var astType = astNode.type;
  var text;
  var childrenResults;
  var vChildren;
  ...
  case 'JSXFragment':
    vProps = {};
    childrenResults = getVNodeChildren(astNode.children, opts, fileState);
    ...
    if (vProps.childFlags) {
      ...
      childFlags = childrenResults.requiresNormalization ? ChildFlags.UnknownChildren : childFlags;
    }
  ...
  case 'JSXElement':
    ...
    vProps = getVNodeProps(openingElement.attributes, vNodeType === TYPE_COMPONENT);
    childrenResults = getVNodeChildren(astNode.children, opts, fileState);
    vChildren = childrenResults.children;
    ...
    if (vNodeType === TYPE_COMPONENT) {
      ...
      if (!childrenResults.requiresNormalization || vProps.childrenKnown) {
        if (vProps.hasKeyedChildren || childrenResults.parentCanBeKeyed) {
          ...
          childrenResults.foundText = false;
          childFlags = ChildFlags.HasTextChildren;
        } else if (childrenResults.hasSingleChild) {
          childFlags = ChildFlags.HasVNodeChildren;
        }
    ...
    if (vChildren && vChildren !== NULL) {
      if (childrenResults.foundText) {
        if (childrenResults.hasSingleChild) {
          ...
          addCreateTextVNodeCalls(vChildren, opts, fileState);
        }
      }
    }

    willNormalizeChildren = vNodeType !== TYPE_COMPONENT && childrenResults.requiresNormalization && !vProps.childrenKnown;
...
function visitorEnter(path, state) {
  var opts = state.opts;
  var node = createVNode(path.node, opts, state.file, path);
    ...
    visitor: {
      Program: {
        enter: function (path, state) {
          ...
          fileState.set('createFragment', false);
        },
        exit: function (path, state) {
          ...
          if (needsAnyImports) {
            ...
            if (optionsImports !== false && optionsImports !== 'false') {
              ...
              if (fileState.get('createFragment')) {
                varArray.push(
                  t.VariableDeclarator(
      ...
      //     state.file.set(path.node.name, VNodeFlags.ComponentFunction);
      //     break;
      //   case 'ClassDeclaration':
      //     state.file.set(path.node.name, VNodeFlags.ComponentClass);
      //     break;
      //   default:
      //     break;
      //   }
