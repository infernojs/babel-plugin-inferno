  ...
  for (var i = 0; i < astProps.length; i++) {
    ...
    if (astProp.type === 'JSXSpreadAttribute') {
      ...
      if (!isComponent && (propName === 'className' || propName === 'class')) {
        ...
        if (propName === 'noNormalize' || propName === 'hasKeyedChildren' || propName === 'hasNonKeyedChildren') {
          // eslint-disable-next-line
          throw path.buildCodeFrameError('Inferno JSX plugin: ' + propName + ' is deprecated use: ' + '$' + propName.charAt(0).toUpperCase() + propName.slice(1));
        }
        switch (propName) {
