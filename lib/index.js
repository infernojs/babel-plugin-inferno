...
function createStaticVElement(t, tag, staticProps, staticChildren, libName) {
	let children = t.identifier('null');

	if (staticChildren.length > 0) {
		children = staticChildren.length === 1 ? staticChildren : t.arrayExpression(staticChildren);
	}
	return t.callExpression(t.memberExpression(t.identifier(libName), t.identifier('createStaticVElement')), [
		t.StringLiteral(tag),
...
function createVElement(
	...
	if (dynamicChildren && dynamicChildren.length > 0) {
		...
		if (childrenType === null) {
			...
			childrenType = t.memberExpression(t.memberExpression(t.identifier(libName), t.identifier('ChildrenTypes')), t.identifier(type));
		}
		pushBpValue(
			t,
			bpValues,
	...
	if (!bpKeyFromMap) {
		...
		bps.push({
			content: [
				createStaticVElement(t, tag, staticProps, staticChildren, libName)
...
function createChildren(t, nodeChildren, bps, templateState, libName, hashBuiler, forceDynamicChildren) {
	...
	for (let i = 0; i < nodeChildren.length; i++) {
		...
		const { isVNode, node, isStatic, empty } = createNode(t, nodeChildren[i], bps, templateState, childHashBuiler, libName, false);
	...
	for (let i = 0; i < staticChildren.length; i++) {
		...
		if (staticChild.staticChildren) {
			staticChildren[i] = createStaticVElement(t, staticChild.name, staticChild.staticProps, staticChild.staticChildren, libName);
		}
	}
	...
	return { allChildrenAreVNodes, staticChildren, dynamicChildren, childrenHash: !isDynamic ? childrenHashes.join('') : '' };
}
...
function createNode(t, node, bps, templateState, hashBuiler, libName, isRoot) {
	switch (node.type) {
		case 'JSXElement':
			...
			if (component) {
				const { dynamicChildren, staticChildren } = createChildren(t, node.children, bps, templateState, libName, hashBuiler, true);
				...
				return {
					...
					isStatic: false,
					empty: false,
					node: createVComponent(t, name, key, ref, dynamicProps, hooks, libName)
				};
			} else {
				const { allChildrenAreVNodes, dynamicChildren, staticChildren, childrenHash } = createChildren(t, node.children, bps, templateState, libName, hashBuiler, false);
				const isStatic = dynamicProps.length === 0 && dynamicChildren.length === 0;
		...
		visitor: {
			JSXElement: {
				enter(path, scope) {
					...
					const { node } = createNode(t, path.node, bps, templateState, { hash: 'tpl' }, libName, true);
