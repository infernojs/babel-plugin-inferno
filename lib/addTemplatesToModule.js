...
function constructTemplate(t, templateElem, parentElem, templateFunc, root, level, index, parentElemName) {
		...
		if (templateElem.children.length > 1) {
			for (var i = 0; i < templateElem.children.length; i++) {
				...
				if(typeof child === "string") {
					...
					constructTemplate(t, templateElem.children[i], templateElem, templateFunc, root, level, i, elemName);
				}
			}
	...
	if (templateElem.attrs && Object.keys(templateElem.attrs).length > 0) {
		...
		templateFunc.push(
			t.ExpressionStatement(
				t.callExpression(
					t.memberExpression(t.identifier("Inferno"), t.identifier(templateElem.component ? addPropsExpression : addAttributesExpression)),
