			...
			if (attrs.index !== undefined) {
				...
				if (attrKeys.length > 0) {
					template.push(t.ObjectProperty(t.identifier('attrs'), t.ObjectExpression(attrKeys.map(function (attrKey) {
						var proposedAttribute = attrKey;
						if (/^(data|aria)/.test(proposedAttribute)){
							proposedAttribute = attrKey.replace(/([A-Z])/g, function(str, match, index) {
								return index ? '-' + match : match;
							}).toLowerCase()
						}
						var id = /[\W]+/.test(proposedAttribute) ? "'" + proposedAttribute + "'" : proposedAttribute;
