...
function constructTemplates(t, node, templates) {
	...
	if (tag) {
		...
		if (spreadKeys && spreadKeys.length > 0) {
			if (spread) {
				const combined = Object.assign({}, attrs, events, hooks);
			...
			if (!isNullOrUndefined(events)) {
				if (eventsKeys.length > 0) {
					...
					values.push(t.ObjectExpression(eventsKeys.map(eventKey => {
						return t.ObjectProperty(t.identifier(eventKey.toLowerCase()), getObjectValue(t, events[eventKey]));
					})));
				}
			}
			if (!isNullOrUndefined(hooks)) {
				if (hooksKeys.length > 0) {
					...
					values.push(t.ObjectExpression(hooksKeys.map(hookKey => {
						return t.ObjectProperty(t.identifier('c' + hookKey.substring(3)), getObjectValue(
							t, hooks[hookKey])
						);
					})));
				}
