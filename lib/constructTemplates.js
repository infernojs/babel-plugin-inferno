...
function constructTemplates(t, node, templates) {
	...
	if (tag) {
		...
		if (!isNullOrUndefined(attrs)) {
			...
			if (attrKeys.length > 0 || (spreadKeys && spreadKeys.length > 0)) {
				...
				if (spread) {
					...
					if (node.staticAttrs) {
						template.push(t.ObjectProperty(t.identifier('attrs'), attrsSpreadObject));
						template.push(t.ObjectProperty(t.identifier('attrKeys'), t.identifier('null')));
					} else {
					...
					if (node.staticAttrs) {
						...
						template.push(t.ObjectProperty(t.identifier('attrKeys'), t.ArrayExpression(attrKeys.map(attrkey => t.StringLiteral(attrkey)))));
					} else {
						template.push(t.ObjectProperty(t.identifier('attrs'), createArgs(t, values.length)));
						template.push(t.ObjectProperty(t.identifier('attrKeys'), t.identifier('null')));
		...
		return t.callExpression(t.identifier('tpl' + node.id), values);
	} else {
